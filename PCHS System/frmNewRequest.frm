...
Begin VB.Form frmNewRequest 
   Caption         =   "New Animal Request"
   ClientHeight    =   7155
   ClientLeft      =   345
   ClientTop       =   930
   ClientWidth     =   6150
   Icon            =   "frmNewRequest.frx":0000
   LinkTopic       =   "Form1"
   ScaleHeight     =   7155
   ScaleWidth      =   6150
   Begin VB.Frame frmRequest 
      ...
      Begin VB.ComboBox cboAge 
         Height          =   315
         ItemData        =   "frmNewRequest.frx":08CA
         Left            =   1440
         List            =   "frmNewRequest.frx":08DA
      ...
      Begin VB.Frame frmSex 
         ...
         Top             =   600
         Width           =   1455
         Begin VB.OptionButton optNoSex 
            Caption         =   "Unspecified"
            Height          =   255
            Left            =   120
            TabIndex        =   37
   ...
   End
End
Attribute VB_Name = "frmNewRequest"
...
Option Explicit
'********************************************************************************************
'* This form is used to enter a new request into the system.  It takes down information about
'* a request and stores it in the reques table.
'*
...
Dim breeds() As combo_info          'Array containing animal breeds
...
'********************************************************************************************
'* Runs when the user clicks save, saves the new animal request to the database
'*
'* Written by: Nick DeSteffen
'* Written on: 10-10-2002
'********************************************************************************************
...
Dim strLicense As String        'Drivers license number of the person

Dim intType As Integer          'Type of animal requested
Dim intBreed As Integer         'Breed of requested animal
Dim strSex As String            'Sex of the requested animal
Dim intColor As Integer         'Color of the requested animal
Dim strAge As String            'Age of the requested animal
...
If cboColor.Text = "" And chkNoColor.Value = 0 Then
    intMsgBox = MsgBox("Please choose the requested color!", vbOKOnly, "Error")
    Exit Sub
End If
...
If cboAge.Text = "" Then
intMsgBox = MsgBox("Please choose the requested age!", vbOKOnly, "Error")
    Exit Sub
End If
...
ElseIf optFemale.Value = True Then
    strSex = "F"
ElseIf optNoSex.Value = True Then
    strSex = "U"
Else
...
If intPersonNum <> 0 And intMsgBox = 6 Then
    ...
    strSQL = "INSERT INTO REQUESTS (REQUEST_PERSON, "
    strSQL = strSQL & "REQUEST_TYPE, "
    strSQL = strSQL & "REQUEST_BREED, "
    strSQL = strSQL & "REQUEST_SEX, "
    strSQL = strSQL & "REQUEST_AGE, "
    strSQL = strSQL & "REQUEST_COLOR) "
    strSQL = strSQL & "VALUES ("
...
ElseIf intMsgBox <> 6 Then
    Exit Sub
Else
    intMsgBox = MsgBox("There was an error, please restart the request!", vbCritical, "Error")
    Exit Sub
End If
    Unload Me
    frmPCHS_Main.Show
...
Dim types() As combo_info           'Array with animal types
Dim colors() As combo_info          'Array with colors
Dim looper As Integer               'Loop control variable
Dim intMsgBox As Integer            'Used for messageboxes
...
Set rstType = New ADODB.Recordset
lblRequestDate.Caption = "Date: " & Date
dtpDOB.Value = Date
looper = 0
...
Private Sub chkNoColor_Click()
If chkNoColor.Value = 1 Then
    cboColor.Enabled = False
ElseIf chkNoColor.Value = 0 Then
